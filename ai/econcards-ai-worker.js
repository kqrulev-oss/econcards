...
const SYSTEM = {
  ...
  similar: 'Ты делаешь КЛОН задачи-образца для тренировки. Меняй ТОЛЬКО числовые значения (цены, количества, коэффициенты, ставки, проценты), сохраняя формулировку, структуру и метод решения полностью идентичными оригиналу — новых сущностей и условий не добавляй. Подбери новые числа так, чтобы задача осталась корректной и с аккуратным ответом (по возможности целым, как в оригинале). Затем реши клон ТЕМ ЖЕ методом, шаг за шагом, и дай ответ. По-русски. Формат: «Задача: …\\n\\nРешение: …\\n\\nОтвет: …».',
};
const PROMPT = {
  ...
  similar: d => `Задача-образец:\n${cut(d.problem)}\n\nЭталонное решение (используй тот же метод):\n${cut(d.reference)}\n\nСделай клон: замени ТОЛЬКО числа и реши заново тем же способом. Ничего в условии, кроме чисел, не меняй.`,
};
